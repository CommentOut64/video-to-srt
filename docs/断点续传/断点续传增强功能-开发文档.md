# 断点续传增强功能 - 开发文档

## 文档版本
- **版本**: v1.0
- **日期**: 2025-01-22
- **状态**: 待实施

---

## 一、概述

本文档详细说明断点续传功能的三个核心增强需求：
1. **支持断点恢复后修改参数**
2. **断点保存未对齐的转录文字**
3. **实现转录文字的流式输出**

---

## 二、需求1：断点恢复后支持修改参数

### 2.1 需求分析

**核心诉求**：

- 用户在恢复断点任务时，应该可以修改某些参数，但不能修改会导致不兼容的参数
- 恢复时强制使用UI面板的当前设置，而非checkpoint中保存的旧设置
- 修改参数时需要明确提示用户可能的后果

### 2.2 参数分类

#### 2.2.1 **禁止修改的参数**（会导致数据不一致）

| 参数 | 原因 | 影响 |
|------|------|------|
| `device` | 虽然理论上可以切换，但不同设备的精度可能有细微差异 | **中等** |
| `word_timestamps` | 如果前半段开启词级时间戳，后半段关闭，会导致SRT格式不一致 | **严重** |

#### 2.2.2 **允许修改但需警告的参数**（可能导致质量不一致）

| 参数 | 原因 | 影响 | 警告级别 |
|------|------|------|----------|
| `model` | 不同模型的输出格式和质量可能不同，混用会导致前后字幕质量不一致。适用场景：用户在转录长视频时发现用错了模型，需要中途更正 | **高** | **严重警告** |

#### 2.2.3 **允许修改的参数**（不影响兼容性）

| 参数 | 原因 | 影响 |
|------|------|------|
| `compute_type` | 仅影响计算精度，不影响输出格式 | **低** |
| `batch_size` | 仅影响处理速度，不影响结果 | **无** |
| `cpu_affinity_*` | 仅影响性能，不影响转录结果 | **无** |

### 2.3 实现方案

#### 2.3.1 数据结构变更

**checkpoint.json 新增字段**：

```json
{
  "job_id": "...",
  "phase": "transcribe",
  "total_segments": 100,
  "processed_indices": [0, 1, 2, ...],
  "segments": [...],
  "unaligned_results": [...],

  // 新增：保存原始设置（用于校验参数兼容性）
  "original_settings": {
    "model": "medium",
    "device": "cuda",
    "word_timestamps": false,
    "compute_type": "float16",  // 可修改
    "batch_size": 16            // 可修改
  }
}
```

#### 2.3.2 后端实现

**文件**: `backend/app/services/transcription_service.py`

**修改 `_save_checkpoint` 方法**：

```python
def _save_checkpoint(self, job_dir: Path, data: dict, job: JobState):
    """
    原子性保存检查点

    Args:
        job_dir: 任务目录
        data: 检查点数据
        job: 任务状态对象（用于获取settings）
    """
    # 添加原始设置到checkpoint
    data["original_settings"] = {
        "model": job.settings.model,
        "device": job.settings.device,
        "word_timestamps": job.settings.word_timestamps,
        "compute_type": job.settings.compute_type,
        "batch_size": job.settings.batch_size
    }

    checkpoint_path = job_dir / "checkpoint.json"
    temp_path = checkpoint_path.with_suffix(".tmp")

    try:
        with open(temp_path, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        os.replace(temp_path, checkpoint_path)
    except Exception as e:
        self.logger.error(f"保存检查点失败: {e}")
```

#### 2.3.3 API契约定义

**新增端点：获取Checkpoint设置**

```
GET /api/checkpoint-settings/{job_id}
```

**响应Schema**：

```json
{
  "has_checkpoint": true,
  "original_settings": {
    "model": "medium",
    "device": "cuda",
    "word_timestamps": false,
    "compute_type": "float16",
    "batch_size": 16
  },
  "progress": {
    "phase": "transcribe",
    "processed": 50,
    "total": 100
  }
}
```

**实现代码**：

```python
# 文件: backend/app/api/routes/transcription_routes.py

@router.get("/checkpoint-settings/{job_id}")
async def get_checkpoint_settings(job_id: str):
    """获取checkpoint中保存的原始设置"""
    from pathlib import Path

    job = transcription_service.get_job(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="任务未找到")

    job_dir = Path(job.dir)
    checkpoint_path = job_dir / "checkpoint.json"

    if not checkpoint_path.exists():
        return {"has_checkpoint": False}

    try:
        with open(checkpoint_path, 'r', encoding='utf-8') as f:
            data = json.load(f)

        return {
            "has_checkpoint": True,
            "original_settings": data.get("original_settings", {}),
            "progress": {
                "phase": data.get("phase"),
                "processed": len(data.get("processed_indices", [])),
                "total": data.get("total_segments", 0)
            }
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"读取检查点失败: {str(e)}")
```

---

## 三、需求2：断点保存未对齐的转录文字

### 3.1 需求分析

**当前问题**：

- 现有代码中，`_transcribe_segment` 方法同时执行了转录（Transcription）和对齐（Alignment）
- checkpoint保存的是已对齐的结果
- 无法在断点续传时跳过已完成的对齐工作

**目标**：

- 将转录和对齐分离为两个独立阶段
- 断点保存未对齐的转录文字（粗略时间轴）
- 恢复时：加载未对齐文字 → 继续转录 → 统一对齐

### 3.2 WhisperX工作流程分析

**当前流程**（在 `_transcribe_segment` 中）：
```python
# 1. Transcription（转录）- 耗时90%
rs = model.transcribe(audio, batch_size=..., language=...)
# rs 包含: {segments: [{start, end, text}], language: 'zh'}
# 此时的 start/end 是粗略的时间戳

# 2. Alignment（对齐）- 耗时10%
aligned = whisperx.align(
    rs['segments'],  # 输入粗略文字
    align_model,
    metadata,
    audio,
    device
)
# aligned 包含: {segments: [{start, end, text}], word_segments: [...]}
# 此时的 start/end 是精确的时间戳
```

**新流程**（分离）：
```
阶段1: 转录（Transcription）
  对每个segment：
    transcribe() → 粗略时间轴的文字
    保存到 checkpoint: unaligned_results

阶段2: 对齐（Alignment）
  等所有segment转录完成后：
    合并所有 unaligned_results
    一次性调用 align() 对整个结果对齐
    生成最终的 aligned_results
```

### 3.3 数据结构设计

#### 3.3.1 checkpoint.json 结构变更

**新增字段**：
```json
{
  "job_id": "...",
  "phase": "transcribe" | "align" | "split",
  "total_segments": 100,
  "processed_indices": [0, 1, 2, ...],
  "segments": [...],  // 音频分段信息

  // 新增：未对齐的转录结果（原始whisper输出）
  "unaligned_results": [
    {
      "segment_index": 0,
      "language": "zh",
      "segments": [
        {
          "id": 0,
          "start": 10.5,    // 粗略时间戳
          "end": 15.2,      // 粗略时间戳
          "text": "这是第一句话"
        }
      ]
    },
    // ...
  ],

  // 删除旧字段：results（已对齐的结果）
  // 对齐结果不需要保存在checkpoint中，因为可以随时重新对齐

  "original_settings": { ... }
}
```

#### 3.3.2 进度阶段新增

**旧阶段**：
```
extract (5%) → split (5%) → transcribe (80%) → srt (10%)
```

**新阶段**：
```
extract (5%) → split (5%) → transcribe (70%) → align (10%) → srt (10%)
```

### 3.4 后端实现

#### 3.4.1 修改 `_transcribe_segment` 方法

**文件**: `backend/app/services/transcription_service.py`

**重命名并简化**：
```python
def _transcribe_segment_unaligned(
    self,
    seg: Dict,
    model,
    job: JobState
) -> Optional[Dict]:
    """
    转录单个音频段（仅转录，不对齐）

    Args:
        seg: 段信息 {file, start_ms, duration_ms}
        model: Whisper模型
        job: 任务状态

    Returns:
        Dict: 未对齐的转录结果
        {
            "segment_index": 0,
            "language": "zh",
            "segments": [{"id": 0, "start": 10.5, "end": 15.2, "text": "..."}]
        }
    """
    audio = whisperx.load_audio(seg['file'])

    try:
        # 仅进行Transcription，不进行Alignment
        rs = model.transcribe(
            audio,
            batch_size=job.settings.batch_size,
            verbose=False,
            language=job.language
        )

        if not rs or 'segments' not in rs:
            return None

        # 检测语言（首次）
        if not job.language and 'language' in rs:
            job.language = rs['language']
            self.logger.info(f"🌐 检测到语言: {job.language}")

        # 时间偏移校正（针对粗略时间戳）
        start_offset = seg['start_ms'] / 1000.0
        adjusted_segments = []

        for idx, s in enumerate(rs['segments']):
            adjusted_segments.append({
                'id': idx,
                'start': s.get('start', 0) + start_offset,
                'end': s.get('end', 0) + start_offset,
                'text': s.get('text', '').strip()
            })

        return {
            'segment_index': seg.get('index', 0),  # 需要在调用时传入
            'language': rs.get('language', job.language),
            'segments': adjusted_segments
        }

    finally:
        del audio
        gc.collect()
```

#### 3.4.2 新增 `_align_all_results` 方法

```python
def _align_all_results(
    self,
    unaligned_results: List[Dict],
    job: JobState,
    audio_path: str
) -> List[Dict]:
    """
    对所有未对齐的转录结果进行统一对齐

    Args:
        unaligned_results: 所有未对齐的转录结果
        job: 任务状态
        audio_path: 完整音频文件路径

    Returns:
        List[Dict]: 对齐后的结果
    """
    self.logger.info(f"🔧 开始统一对齐 {len(unaligned_results)} 个分段的转录结果")

    # 1. 合并所有segments
    all_segments = []
    for result in unaligned_results:
        all_segments.extend(result['segments'])

    if not all_segments:
        self.logger.warning("没有可对齐的内容")
        return []

    # 2. 加载完整音频
    audio = whisperx.load_audio(audio_path)

    try:
        # 3. 获取对齐模型
        lang = job.language or unaligned_results[0].get('language', 'zh')
        align_model, metadata = self._get_align_model(lang, job.settings.device, job)

        # 4. 执行对齐（一次性处理所有segments）
        self._update_progress(job, 'align', 0, '正在对齐时间轴...')

        aligned = whisperx.align(
            all_segments,
            align_model,
            metadata,
            audio,
            job.settings.device
        )

        self._update_progress(job, 'align', 1, '对齐完成')

        # 5. 返回对齐后的结果
        return [{
            'segments': aligned.get('segments', []),
            'word_segments': aligned.get('word_segments', [])
        }]

    finally:
        del audio
        gc.collect()
```

#### 3.4.3 修改 `_run_pipeline` 方法

```python
def _run_pipeline(self, job: JobState):
    """
    执行转录处理管道（支持断点续传 + 分离转录和对齐）
    """
    # ... 前置代码 ...

    try:
        # ==========================================
        # 阶段1-2: 音频提取和分段（不变）
        # ==========================================
        # ... 代码省略 ...

        # ==========================================
        # 阶段3: 转录处理（仅转录，不对齐）
        # ==========================================

        # 初始化未对齐结果
        unaligned_results = []

        # 尝试从checkpoint恢复
        if checkpoint and 'unaligned_results' in checkpoint:
            unaligned_results = checkpoint['unaligned_results']
            self.logger.info(f"✅ 从检查点恢复 {len(unaligned_results)} 个已转录段")

        self._update_progress(job, 'transcribe', 0, '加载模型中')
        model = self._get_model(job.settings, job)

        # 过滤出需要处理的段
        for idx, seg in enumerate(current_segments):
            if idx in processed_indices:
                self.logger.debug(f"⏭️ 跳过已转录段 {idx}")
                continue

            if job.canceled or job.paused:
                raise RuntimeError('任务已取消' if job.canceled else '任务已暂停')

            # 添加segment索引
            seg['index'] = idx

            # 仅转录，不对齐
            result = self._transcribe_segment_unaligned(seg, model, job)

            if result:
                unaligned_results.append(result)

            # 更新进度
            processed_indices.add(idx)
            job.processed = len(processed_indices)

            progress = len(processed_indices) / len(current_segments)
            self._update_progress(
                job, 'transcribe', progress,
                f'转录中 {len(processed_indices)}/{len(current_segments)}'
            )

            # 保存checkpoint（包含未对齐结果）
            checkpoint_data = {
                "job_id": job.job_id,
                "phase": "transcribe",
                "total_segments": len(current_segments),
                "processed_indices": list(processed_indices),
                "segments": current_segments,
                "unaligned_results": unaligned_results  # 保存未对齐结果
            }
            self._save_checkpoint(job_dir, checkpoint_data, job)

        self._update_progress(job, 'transcribe', 1, '转录完成')

        # ==========================================
        # 阶段4: 统一对齐
        # ==========================================

        self._update_progress(job, 'align', 0, '准备对齐...')

        aligned_results = self._align_all_results(
            unaligned_results,
            job,
            str(audio_path)
        )

        # ==========================================
        # 阶段5: 生成SRT
        # ==========================================

        base_name = os.path.splitext(job.filename)[0]
        srt_path = job_dir / f'{base_name}.srt'

        self._update_progress(job, 'srt', 0, '生成字幕文件...')

        self._generate_srt(
            aligned_results,
            str(srt_path),
            job.settings.word_timestamps
        )

        self._update_progress(job, 'srt', 1, '处理完成')

        job.srt_path = str(srt_path)

        # 清理checkpoint
        try:
            checkpoint_file = job_dir / "checkpoint.json"
            checkpoint_file.unlink(missing_ok=True)
            self.logger.info("🧹 检查点已清理")
        except Exception as e:
            self.logger.warning(f"清理检查点失败: {e}")

        job.status = 'finished'
        job.message = '完成'

    except Exception as e:
        # ... 异常处理 ...
```

### 3.5 新增API端点：提取转录文字

**文件**: `backend/app/api/routes/transcription_routes.py`

```python
@router.get("/transcription-text/{job_id}")
async def get_transcription_text(job_id: str):
    """
    从checkpoint中提取已完成的转录文字（未对齐版本）

    返回格式：
    {
        "job_id": "...",
        "has_checkpoint": true,
        "language": "zh",
        "segments": [
            {"id": 0, "start": 10.5, "end": 15.2, "text": "第一句话"},
            {"id": 1, "start": 15.2, "end": 20.0, "text": "第二句话"}
        ],
        "progress": {
            "processed": 50,
            "total": 100,
            "percentage": 50.0
        }
    }
    """
    from pathlib import Path

    job = transcription_service.get_job(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="任务未找到")

    job_dir = Path(job.dir)
    checkpoint_path = job_dir / "checkpoint.json"

    if not checkpoint_path.exists():
        return {
            "job_id": job_id,
            "has_checkpoint": False,
            "message": "没有检查点数据"
        }

    try:
        with open(checkpoint_path, 'r', encoding='utf-8') as f:
            data = json.load(f)

        # 提取未对齐结果
        unaligned_results = data.get("unaligned_results", [])

        # 合并所有segments
        all_segments = []
        for result in unaligned_results:
            all_segments.extend(result.get('segments', []))

        # 按时间排序
        all_segments.sort(key=lambda x: x.get('start', 0))

        # 重新编号
        for idx, seg in enumerate(all_segments):
            seg['id'] = idx

        return {
            "job_id": job_id,
            "has_checkpoint": True,
            "language": data.get("unaligned_results", [{}])[0].get("language", "unknown"),
            "segments": all_segments,
            "progress": {
                "processed": len(data.get("processed_indices", [])),
                "total": data.get("total_segments", 0),
                "percentage": round(
                    len(data.get("processed_indices", [])) / max(1, data.get("total_segments", 1)) * 100,
                    2
                )
            }
        }

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"读取转录文字失败: {str(e)}")
```

### 3.6 配置更新

**文件**: `backend/app/core/config.py`

```python
# 进度权重配置（新增align阶段）
PHASE_WEIGHTS = {
    "extract": 5,
    "split": 5,
    "transcribe": 70,  # 从80降到70
    "align": 10,       # 新增
    "srt": 10
}
TOTAL_WEIGHT = sum(PHASE_WEIGHTS.values())  # 100
```

---

## 四、需求3：实现转录文字的流式输出

### 4.1 需求分析

**目标**：
- 转录每完成一个片段，立即将未对齐文字推送到前端
- 用户可以实时查看和修改转录文字
- 对齐完成后，瞬间更新所有文字（保留用户修改）

**技术选型**：

- **通信方式**: SSE (Server-Sent Events)
- **前端展示**: 虚拟滚动列表 (vue-virtual-scroller)
- **数据管理**: Pinia store + shallowRef
- **用户修改**: is_modified 脏标记机制

### 4.2 架构设计

```
┌─────────────────────────────────────────────────────────────┐
│                       后端架构                                │
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  转录循环                     SSE推送                          │
│  ┌─────────────┐            ┌──────────────┐                │
│  │ segment_0   │──转录完成──→│ type: segment │                │
│  └─────────────┘            │ data: {...}   │───推送─┐       │
│                             └──────────────┘        │       │
│  ┌─────────────┐            ┌──────────────┐        │       │
│  │ segment_1   │──转录完成──→│ type: segment │        │       │
│  └─────────────┘            │ data: {...}   │───推送─┤       │
│                             └──────────────┘        │       │
│       ...                         ...               │       │
│                                                      │       │
│  统一对齐                                             │       │
│  ┌─────────────┐            ┌──────────────┐        │       │
│  │ align_all   │──对齐完成──→│ type: aligned │        │       │
│  └─────────────┘            │ data: [...]   │───推送─┤       │
│                             └──────────────┘        │       │
└─────────────────────────────────────────────────────┼───────┘
```

### 4.3 后端实现

#### 4.3.1 SSE推送服务

**文件**: `backend/app/services/sse_service.py` (新建)

```python
"""
SSE (Server-Sent Events) 推送服务
"""
import asyncio
import json
import logging
from typing import Dict, Optional
from collections import defaultdict
from fastapi import Request

logger = logging.getLogger(__name__)


class SSEManager:
    """SSE连接管理器"""

    def __init__(self):
        # 每个job_id可能有多个客户端连接
        self.connections: Dict[str, list] = defaultdict(list)

    async def subscribe(self, job_id: str, request: Request):
        """
        订阅任务的SSE事件流

        Args:
            job_id: 任务ID
            request: FastAPI请求对象

        Yields:
            SSE格式的消息
        """
        # 创建队列
        queue = asyncio.Queue()
        self.connections[job_id].append(queue)

        try:
            logger.info(f"SSE客户端已连接: job_id={job_id}")

            # 发送连接成功消息
            yield self._format_sse({
                "type": "connected",
                "job_id": job_id,
                "message": "SSE连接已建立"
            })

            # 持续推送消息
            while True:
                # 检查客户端是否断开
                if await request.is_disconnected():
                    logger.info(f"SSE客户端已断开: job_id={job_id}")
                    break

                try:
                    # 等待新消息（超时10秒发送心跳）
                    message = await asyncio.wait_for(queue.get(), timeout=10.0)
                    yield self._format_sse(message)
                except asyncio.TimeoutError:
                    # 发送心跳保持连接
                    yield ": heartbeat\n\n"

        finally:
            # 清理连接
            self.connections[job_id].remove(queue)
            if not self.connections[job_id]:
                del self.connections[job_id]
            logger.info(f"SSE连接已清理: job_id={job_id}")

    async def broadcast(self, job_id: str, message: dict):
        """
        向指定任务的所有订阅者广播消息

        Args:
            job_id: 任务ID
            message: 消息内容
        """
        if job_id not in self.connections:
            return

        for queue in self.connections[job_id]:
            try:
                await queue.put(message)
            except Exception as e:
                logger.error(f"SSE推送失败: {e}")

    def _format_sse(self, data: dict) -> str:
        """
        格式化为SSE消息格式

        Args:
            data: 数据字典

        Returns:
            SSE格式字符串
        """
        return f"data: {json.dumps(data, ensure_ascii=False)}\n\n"


# 全局单例
_sse_manager: Optional[SSEManager] = None


def get_sse_manager() -> SSEManager:
    """获取SSE管理器单例"""
    global _sse_manager
    if _sse_manager is None:
        _sse_manager = SSEManager()
    return _sse_manager
```

#### 4.3.2 集成SSE推送到转录服务

**文件**: `backend/app/services/transcription_service.py`

```python
# 在 __init__ 中添加
def __init__(self, jobs_root: str):
    # ... 现有代码 ...

    # 集成SSE管理器
    from services.sse_service import get_sse_manager
    self.sse_manager = get_sse_manager()


# 在 _run_pipeline 中的转录循环添加推送
def _run_pipeline(self, job: JobState):
    # ... 前置代码 ...

    # 在转录循环中
    for idx, seg in enumerate(current_segments):
        if idx in processed_indices:
            continue

        # 转录
        result = self._transcribe_segment_unaligned(seg, model, job)

        if result:
            unaligned_results.append(result)

            # 【新增】立即推送到前端
            asyncio.create_task(self.sse_manager.broadcast(
                job.job_id,
                {
                    "type": "segment",
                    "data": {
                        "segment_index": idx,
                        "segments": result['segments'],
                        "language": result.get('language'),
                        "progress": {
                            "processed": len(processed_indices) + 1,
                            "total": len(current_segments),
                            "percentage": round(
                                (len(processed_indices) + 1) / len(current_segments) * 100,
                                2
                            )
                        }
                    }
                }
            ))

        # 更新processed_indices和保存checkpoint
        processed_indices.add(idx)
        # ...

    # 对齐完成后推送
    aligned_results = self._align_all_results(...)

    # 【新增】推送对齐完成事件
    asyncio.create_task(self.sse_manager.broadcast(
        job.job_id,
        {
            "type": "aligned",
            "data": {
                "segments": aligned_results[0]['segments'],
                "word_segments": aligned_results[0].get('word_segments', []),
                "message": "对齐完成"
            }
        }
    ))
```

#### 4.3.3 SSE API端点

**文件**: `backend/app/api/routes/transcription_routes.py`

```python
from fastapi import Request
from fastapi.responses import StreamingResponse
from services.sse_service import get_sse_manager

sse_manager = get_sse_manager()


@router.get("/stream/{job_id}")
async def stream_transcription(job_id: str, request: Request):
    """
    订阅任务的SSE事件流

    事件类型：
    - connected: 连接建立
    - segment: 单个片段转录完成
    - aligned: 全部对齐完成
    - error: 错误事件
    """
    job = transcription_service.get_job(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="任务未找到")

    return StreamingResponse(
        sse_manager.subscribe(job_id, request),
        media_type="text/event-stream",
        headers={
            "Cache-Control": "no-cache",
            "Connection": "keep-alive",
            "X-Accel-Buffering": "no"  # 禁用nginx缓冲
        }
    )
```

**注意**：前端如何使用这些API进行参数校验、SSE监听、字幕编辑等功能，请参考《新界面开发.md》文档。

---

## 五、实施步骤

### 5.1 优先级排序

| 功能 | 优先级 | 预计工时 | 依赖 |
|------|--------|----------|------|
| 需求2：分离转录和对齐 | **P0** | 6小时 | 无 |
| 需求1：参数修改校验 | **P1** | 4小时 | 需求2 |
| 需求3：基础SSE推送 | **P1** | 8小时 | 需求2 |
| 需求3：性能优化（Worker） | **P3** | 4小时 | 需求3前端 |

### 5.2 实施顺序

**第一阶段（核心功能）**：

1. 修改checkpoint数据结构，添加`unaligned_results`和`original_settings`字段
2. 重构`_transcribe_segment`为`_transcribe_segment_unaligned`
3. 实现`_align_all_results`方法
4. 修改`_run_pipeline`，分离转录和对齐阶段
5. 更新进度权重配置
6. 测试断点续传是否正常工作

**第二阶段（参数校验）**：
1. 实现后端checkpoint设置保存逻辑
2. 添加`/checkpoint-settings/{job_id}`API端点
3. 前端实现参数校验逻辑
4. 集成到恢复任务流程
5. 测试参数修改提示和校验

**第三阶段（流式输出）**：
1. 创建SSE服务（`sse_service.py`）
2. 在转录服务中集成SSE推送
3. 添加`/stream/{job_id}`API端点

**第四阶段（优化和完善）**：

1. 添加Web Worker支持（可选）
2. 性能测试和优化
3. 完善错误处理和边界情况

---

## 六、测试计划

### 6.1 功能测试

**需求1：参数修改**
- [ ] 恢复任务时修改禁止参数，应强制恢复原值
- [ ] 恢复任务时修改允许参数，应弹出提示
- [ ] 用户确认修改后，使用新参数继续转录
- [ ] 用户取消修改，恢复为原参数

**需求2：转录保存**
- [ ] 转录过程中断，checkpoint包含未对齐结果
- [ ] 恢复任务后继续转录剩余片段
- [ ] 最后统一对齐所有结果
- [ ] 调用`/transcription-text/{job_id}`能正确提取文字

**需求3：流式输出**

- [ ] SSE连接成功建立
- [ ] 每转录完一个片段立即推送到前端
- [ ] 用户修改字幕后，is_modified标记正确
- [ ] 对齐完成后，未修改字幕更新，已修改字幕保留
- [ ] 导出SRT包含用户的所有修改

### 6.2 性能测试

- [ ] 2小时视频（约2000条字幕）的渲染性能
- [ ] SSE推送频率测试（每秒1-2条消息）
- [ ] 对齐完成时的合并性能（2000条数据）
- [ ] 内存占用监控

### 6.3 边界测试

- [ ] SSE连接意外断开的处理
- [ ] 转录过程中切换任务
- [ ] 暂停后断开SSE，恢复后重新连接
- [ ] checkpoint文件损坏的处理
- [ ] 参数校验异常处理

---

## 七、兼容性保障

### 7.1 向后兼容

**旧checkpoint格式兼容**：
```python
# 加载checkpoint时兼容旧格式
checkpoint = self._load_checkpoint(job_dir)

if checkpoint:
    # 旧格式：results字段（已对齐）
    if 'results' in checkpoint and 'unaligned_results' not in checkpoint:
        self.logger.warning("检测到旧版checkpoint格式，将跳过对齐阶段")
        # 直接使用results作为最终结果
        final_results = checkpoint['results']
    # 新格式：unaligned_results字段
    elif 'unaligned_results' in checkpoint:
        unaligned_results = checkpoint['unaligned_results']
        # 继续新流程
```

### 7.2 渐进式增强

- SSE功能可选：如果前端不连接SSE，不影响基本转录功能
- 字幕编辑器可选：可以保留旧的进度条UI，新增编辑器作为可选功能
- 参数校验可选：如果前端不调用校验API，后端仍正常工作

## 九、附录

### 9.1 相关文件清单

**后端新建文件**：
- `backend/app/services/sse_service.py` - SSE推送服务

**后端修改文件**：
- `backend/app/services/transcription_service.py` - 核心转录服务
- `backend/app/api/routes/transcription_routes.py` - API路由
- `backend/app/core/config.py` - 配置文件

**前端实现**：所有前端文件的创建和修改请参考《新界面开发.md》文档。

### 9.2 依赖包

**前端新增**：
```json
{
  "dependencies": {
    "vue-virtual-scroller": "^2.0.0-beta.8"
  }
}
```

**后端无新增依赖**（使用FastAPI内置的StreamingResponse）

---

**文档结束**
