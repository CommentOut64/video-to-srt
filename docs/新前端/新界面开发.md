# Video-to-SRT Pro：前端重构开发文档 (v2.1)

## 1\. 项目愿景与技术栈

  * **核心理念**：Local-First（本地优先）、生产力工具、非线性编辑。
  * **交互模式**：工作台模式（Workbench）。
  * **技术栈升级**：
      * **框架**：Vue 3 (Composition API) + Vite
      * **状态管理**：Pinia (单一种源 Single Source of Truth)
      * **UI 框架**：Element Plus (容器/交互组件) + **SCSS (模块化样式)**
      * **核心库**：
          * **音视频可视化**：`wavesurfer.js` (V7) —— *注：采用 Backend Peaks 模式以提升长音频性能。*
          * **长列表优化**：`vue-virtual-scroller`
          * **工具库**：`@vueuse/core` (深色模式、快捷键、ResizeObserver)

-----

## 2\. 核心数据结构 (Pinia Store 设计)

前端并不直接操作 SRT 字符串，而是维护一个响应式的对象数组。

```javascript
// stores/projectStore.js

state: () => ({
  // 1. 项目元数据
  meta: {
    jobId: null,
    videoPath: null,      // 视频源文件路径 (用于 <video>)
    audioPath: null,      // 音频源文件路径 (用于 wavesurfer)
    peaksPath: null,      // 波形数据文件路径 (新增：后端预计算的波形数据)
    duration: 0,
    lastSaved: Date.now()
  },

  // 1.5 任务状态管理（支持断点续传）
  task: {
    status: 'idle',       // 'idle' | 'transcribing' | 'aligning' | 'finished' | 'error' | 'paused'
    progress: 0,
    currentSegment: 0,
    totalSegments: 0,
    phase: '',            // 'transcribe' | 'align'
    streaming: false,     // 是否正在接收 SSE 流
    hasCheckpoint: false, // 是否有断点可恢复
    originalSettings: {}, // 断点中保存的原始配置
  },

  // 2. 字幕核心数据 (Single Source of Truth)
  // 使用 Map 或以 ID 为 Key 的对象可能比数组性能更好，但数组对虚拟列表更友好
  subtitles: [
    {
      id: "uuid-1",       // 唯一标识符
      start: 1.500,       // 秒 (float, 3位小数)
      end: 3.200,
      text: "Hello World",
      status: 'confirmed',  // 新增：'draft'(未对齐) | 'confirmed'(已对齐/人工修改)
      isDirty: false      // 单行脏状态（用户是否修改过）
    },
    // ...
  ],

  // 3. 播放器全局状态 (用于组件间同步)
  player: {
    currentTime: 0,
    isPlaying: false,
    playbackRate: 1.0,
    volume: 1.0
  },

  // 4. 视图状态
  view: {
    theme: 'dark',        // 'dark' | 'light'
    zoomLevel: 100,       // 波形缩放比例
    autoScroll: true      // 列表自动跟随
  }
})

actions: {
  // 核心操作需封装为 Action 以便于做 撤销/重做 (Undo/Redo)
  updateSubtitle(id, payload) { ... },
  addSubtitle(index, payload) { ... },
  removeSubtitle(id) { ... },

  // 导入导出
  parseSRT(rawString) { ... },
  generateSRT() { ... },

  // ========== 新增：断点续传与流式输出支持 ==========

  /**
   * SSE流式数据接收 Action
   * 处理从后端推送过来的 'segment' (未对齐) 和 'aligned' (已对齐) 事件
   */
  handleStreamEvent(event) {
    if (event.type === 'segment') {
      // 实时追加未对齐的草稿片段
      // 前端显示为灰色或斜体，表示"正在生成中"
      event.data.segments.forEach(seg => {
        this.subtitles.push({
          ...seg,
          status: 'draft'  // 标记为草稿状态
        })
      })
      this.task.progress = event.data.progress.percentage
      this.task.currentSegment = event.data.progress.processed
      this.task.totalSegments = event.data.progress.total
    }
    else if (event.type === 'aligned') {
      // 对齐完成：批量更新时间轴，但保留用户在流式传输期间可能手动修改的文本
      this.mergeAlignedSubtitles(event.data.segments)
      this.task.phase = 'align'
      this.task.status = 'finished'
    }
    else if (event.type === 'connected') {
      this.task.streaming = true
    }
  },

  /**
   * 智能合并对齐结果 (保留用户修改)
   * 如果数据量>5000条，建议使用Web Worker处理
   */
  mergeAlignedSubtitles(alignedData) {
    // 实现逻辑：遍历当前 subtitles
    // 如果用户手动修改过(isDirty=true)，则保留文本只更新时间轴
    // 否则完全替换为对齐后的高精度数据
    this.subtitles = this.subtitles.map(item => {
      const aligned = alignedData.find(a => a.id === item.id)
      if (!aligned) return item

      if (item.isDirty) {
        // 保留用户修改的文本，仅更新时间轴
        return {
          ...item,
          start: aligned.start,
          end: aligned.end,
          status: 'confirmed'
          // text 保持不变
        }
      } else {
        // 完全信任AI的对齐结果
        return {
          ...item,
          start: aligned.start,
          end: aligned.end,
          text: aligned.text,
          status: 'confirmed'
        }
      }
    })
  },

  /**
   * 加载断点信息（用于恢复任务）
   */
  async loadCheckpoint(jobId) {
    try {
      const response = await fetch(`/api/checkpoint-settings/${jobId}`)
      const data = await response.json()

      if (data.has_checkpoint) {
        this.task.hasCheckpoint = true
        this.task.originalSettings = data.original_settings
        this.task.currentSegment = data.progress.processed
        this.task.totalSegments = data.progress.total
        this.task.progress = (data.progress.processed / data.progress.total) * 100
        return data
      }
      return null
    } catch (error) {
      console.error('加载断点失败:', error)
      return null
    }
  },

  /**
   * 参数校验（对比当前UI设置和断点中的原始设置）
   * 返回: { valid: boolean, changes: array, warningLevel: string }
   */
  validateSettingsChange(currentSettings, originalSettings) {
    const immutableParams = ['device', 'word_timestamps']
    const warnParams = ['model']  // 允许修改但需严重警告
    const freeParams = ['compute_type', 'batch_size', 'cpu_affinity_enabled', 'cpu_affinity_strategy']

    const changedImmutable = []
    const changedWarn = []
    const changedFree = []

    // 检查禁止修改的参数
    for (const key of immutableParams) {
      if (currentSettings[key] !== originalSettings[key]) {
        changedImmutable.push({
          key,
          old: originalSettings[key],
          new: currentSettings[key]
        })
      }
    }

    // 检查需要警告的参数
    for (const key of warnParams) {
      if (currentSettings[key] !== originalSettings[key]) {
        changedWarn.push({
          key,
          old: originalSettings[key],
          new: currentSettings[key]
        })
      }
    }

    // 检查自由修改的参数
    for (const key of freeParams) {
      if (currentSettings[key] !== originalSettings[key]) {
        changedFree.push({
          key,
          old: originalSettings[key],
          new: currentSettings[key]
        })
      }
    }

    return {
      valid: changedImmutable.length === 0,
      immutableChanges: changedImmutable,
      warnChanges: changedWarn,
      freeChanges: changedFree,
      warningLevel: changedImmutable.length > 0 ? 'error' :
                    changedWarn.length > 0 ? 'warning' :
                    changedFree.length > 0 ? 'info' : 'none'
    }
  }
}
```

-----

## 3\. 页面布局与样式规范

### 3.1 样式工程化

  * **预处理器**：使用 **SCSS**。
  * **模块化**：采用 Vue Scoped CSS + BEM 命名规范，或者 CSS Modules，防止样式污染。
  * **响应式设计**：
      * **断点**：`Desktop` (\>1200px), `Laptop` (992px-1200px), `Tablet` (\<992px)。
      * **策略**：在小屏幕下，隐藏右侧字幕列表（改为抽屉式 Drawer 呼出），优先保证视频和波形的可操作区域。
  * **主题定制 (Theming)**：
      * 基于 CSS Variables 定义语义化颜色（如 `--bg-primary`, `--text-normal`）。
      * 利用 `@vueuse/core` 的 `useDark` 实现一键切换 **深色/浅色** 模式。
      * **默认深色**：适合沉浸式编辑；**可选浅色**：适合强光环境下校对。

### 3.2 EditorView (编辑器) 布局详情

采用 **Flex/Grid 混合布局**，确保在窗口缩放时各区域比例协调。

#### **A. 顶部工具栏 (Header)**

  * **左**：Logo、文件名、保存状态（自动保存中... / 已保存）。
  * **右**：
      * **主题切换**：☀️/🌙 图标。
      * **导出**：SRT/TXT/视频压制。
      * **设置**：打开设置模态窗。

#### **B. 中部工作区 (Workspace)** - *支持拖拽改变分栏宽度*

  * **左侧：视频监视器 (Video Monitor)**
      * **自适应**：保持 16:9 或视频原始比例，居中显示。
      * **交互**：支持滚轮缩放 (Zoom)、空格拖拽 (Pan)。
  * **右侧：脚本编辑器 (Script Editor)**
      * **双模式切换**：[列表模式] (默认) | [文稿模式] (适合大段修改)。
      * **响应式**：在窄屏模式下自动折叠，点击图标侧滑展开。

#### **C. 底部控制台 (Bottom Console)**

  * **播放控制条**：
      * **倍速控制**：不再仅限于下拉菜单，提供 **自定义输入框** (0.5x - 4.0x) 和 **滑块**，满足极限校对需求。
      * **音量/静音**。
  * **波形时间轴**：
      * 全宽显示，高度自适应（建议至少 150px）。

-----

## 4\. 核心组件技术详解与优化

### 4.1 后端接口增强 (配合长音频优化)

为了解决 `wavesurfer.js` 解析长音频（1小时+）导致的浏览器卡死问题，必须由后端预计算波形数据。

  * **新增接口**：`GET /api/media/{job_id}/peaks`
  * **功能**：后端使用 `audiowaveform` (C++工具) 或 Python `librosa` 快速提取音频的峰值数据 (JSON/Dat 格式)。
  * **前端逻辑**：`wavesurfer.load(audioUrl, peaksData)`。这样无论音频多长，波形图都是瞬间渲染出来的。

### 4.2 WaveformTimeline (波形时间轴) - *增强交互*

  * **技术选型**：`wavesurfer.js` + `Regions Plugin` + `Timeline Plugin`。
  * **同步拖拽 (Sync Dragging)**：
      * **功能**：在波形上拖拽 Region 的边缘（调整时间）时，右侧字幕列表对应的时间输入框数值**实时跳动更新**。
      * **反向同步**：在右侧列表修改时间数字时，波形上的 Region **实时位移**。
  * **性能优化**：
      * 启用 `backend` 模式（MediaElement），复用 `<video>` 标签的音频轨道，避免加载两份大文件。

### 4.3 异常处理与鲁棒性 (Robustness)

设计一个全局的 `ErrorHandler` 机制：

1.  **资源加载失败**：
      * 视频/音频 404：显示"资源丢失"占位图，并提供"重新定位文件"的按钮。
      * 波形数据生成失败：前端自动降级，尝试在浏览器端解码音频（显示 Loading 进度条）。
2.  **操作保护**：
      * **未保存拦截**：检测 `store.isDirty`，在关闭页面/刷新前弹出 `beforeunload` 警告。
      * **自动保存 (Auto-Save)**：每 30 秒将当前状态写入 `localStorage` 或后端临时文件，防止崩坏丢失。
3.  **错误提示 UI**：
      * 使用 `ElNotification` 显示非阻断式错误（如"自动保存失败，请检查网络"）。
      * 关键错误（如"WebAssembly 崩溃"）使用模态窗引导刷新。

### 4.4 断点续传与流式响应机制 (Integration)

配合后端断点续传增强功能（详见《断点续传增强功能-开发文档.md》），前端需实现复杂的握手与渲染逻辑。

#### **A. 启动/恢复时的参数校验 (Handshake)**

在调用恢复接口前，必须先进行"参数兼容性检查"：

1. **获取 Checkpoint**：调用 `GET /api/checkpoint-settings/{jobId}`。
2. **比对逻辑**：
   * **禁止修改参数** (`device`, `word_timestamps`)：如果用户在UI中修改了这些参数，强制恢复为原值并警告用户。
   * **警告修改参数** (`model`)：弹出警告对话框，明确告知用户"修改模型会导致前后字幕质量不一致，是否继续？"
   * **自由修改参数** (`compute_type`, `batch_size`, `cpu_affinity_*`)：提示用户参数已修改，但允许继续。

**实现示例**：

```javascript
// composables/useResumeValidation.js

import { ElMessageBox, ElMessage } from 'element-plus'
import { useProjectStore } from '@/stores/projectStore'

export function useResumeValidation() {
  const store = useProjectStore()

  /**
   * 恢复任务前的参数校验
   * @param {string} jobId - 任务ID
   * @param {Object} currentSettings - 当前UI面板的设置
   * @returns {Promise<boolean>} - 是否允许继续
   */
  async function validateResume(jobId, currentSettings) {
    // 1. 加载断点信息
    const checkpoint = await store.loadCheckpoint(jobId)
    if (!checkpoint || !checkpoint.has_checkpoint) {
      return true  // 没有断点，直接继续
    }

    // 2. 校验参数变更
    const validation = store.validateSettingsChange(
      currentSettings,
      checkpoint.original_settings
    )

    // 3. 根据校验结果采取不同措施
    if (validation.warningLevel === 'error') {
      // 禁止修改的参数被修改了
      await ElMessageBox.alert(
        `以下关键参数不能修改，将自动恢复为原始值：\n` +
        validation.immutableChanges.map(c =>
          `- ${c.key}: ${c.old} → ${c.new}`
        ).join('\n'),
        '参数修改限制',
        { type: 'error' }
      )

      // 强制恢复原值
      validation.immutableChanges.forEach(change => {
        currentSettings[change.key] = change.old
      })
      return false  // 需要用户重新确认
    }

    if (validation.warningLevel === 'warning') {
      // model参数被修改，严重警告
      try {
        await ElMessageBox.confirm(
          `⚠️ 检测到模型参数已修改：\n` +
          validation.warnChanges.map(c =>
            `- ${c.key}: ${c.old} → ${c.new}`
          ).join('\n') +
          `\n\n修改模型会导致前后字幕质量不一致！\n` +
          `适用场景：发现用错了模型需要中途更正。\n\n` +
          `是否继续使用新模型？`,
          '严重警告',
          {
            confirmButtonText: '继续（使用新模型）',
            cancelButtonText: '恢复原模型',
            type: 'warning',
            dangerouslyUseHTMLString: true
          }
        )
        return true  // 用户确认，允许继续
      } catch {
        // 用户取消，恢复原设置
        validation.warnChanges.forEach(change => {
          currentSettings[change.key] = change.old
        })
        ElMessage.info('已恢复为原始模型配置')
        return false
      }
    }

    if (validation.warningLevel === 'info') {
      // 自由修改的参数变更，友好提示
      ElMessage.info(
        `检测到参数修改：${validation.freeChanges.map(c => c.key).join(', ')}`
      )
      return true
    }

    return true  // 无变更，直接继续
  }

  return {
    validateResume
  }
}
```

#### **B. SSE 流式监听封装 (useTranscribeStream)**

创建一个 Composable 封装 SSE 连接逻辑，自动处理重连、错误恢复等。

```javascript
// composables/useTranscribeStream.js

import { ref, onUnmounted } from 'vue'
import { useProjectStore } from '@/stores/projectStore'
import { ElMessage } from 'element-plus'

export function useTranscribeStream() {
  const store = useProjectStore()
  const eventSource = ref(null)
  const isConnected = ref(false)

  /**
   * 连接SSE流
   * @param {string} jobId - 任务ID
   */
  function connect(jobId) {
    disconnect()  // 先断开旧连接

    const url = `/api/stream/${jobId}`
    eventSource.value = new EventSource(url)

    eventSource.value.onmessage = (event) => {
      try {
        const data = JSON.parse(event.data)
        handleMessage(data)
      } catch (error) {
        console.error('SSE消息解析失败:', error)
      }
    }

    eventSource.value.onerror = (error) => {
      console.error('SSE连接错误:', error)
      isConnected.value = false

      // 自动重连逻辑（可选）
      setTimeout(() => {
        if (store.task.status === 'transcribing') {
          console.log('SSE重连中...')
          connect(jobId)
        }
      }, 5000)
    }

    console.log(`SSE已连接: ${url}`)
  }

  /**
   * 处理SSE消息
   */
  function handleMessage(data) {
    switch (data.type) {
      case 'connected':
        isConnected.value = true
        store.task.streaming = true
        console.log('SSE连接已建立')
        break

      case 'segment':
        // 新片段转录完成 - 流式追加
        store.handleStreamEvent(data)
        ElMessage.success({
          message: `已转录 ${data.data.progress.processed}/${data.data.progress.total} 段`,
          duration: 1000,
          showClose: false
        })
        break

      case 'aligned':
        // 对齐完成 - 批量更新
        store.handleStreamEvent(data)
        ElMessage.success({
          message: '✅ 对齐完成！字幕已更新',
          duration: 3000
        })
        break

      case 'error':
        console.error('SSE错误事件:', data.message)
        ElMessage.error(`转录错误: ${data.message}`)
        break

      default:
        console.warn('未知的SSE事件类型:', data.type)
    }
  }

  /**
   * 断开连接
   */
  function disconnect() {
    if (eventSource.value) {
      eventSource.value.close()
      eventSource.value = null
      isConnected.value = false
      console.log('SSE已断开')
    }
  }

  // 组件卸载时自动断开
  onUnmounted(() => {
    disconnect()
  })

  return {
    connect,
    disconnect,
    isConnected
  }
}
```

#### **C. 使用示例（在 EditorView 中集成）**

```vue
<script setup>
import { ref, onMounted } from 'vue'
import { useProjectStore } from '@/stores/projectStore'
import { useResumeValidation } from '@/composables/useResumeValidation'
import { useTranscribeStream } from '@/composables/useTranscribeStream'

const store = useProjectStore()
const { validateResume } = useResumeValidation()
const { connect, disconnect } = useTranscribeStream()

const currentSettings = ref({
  model: 'medium',
  device: 'cuda',
  word_timestamps: false,
  compute_type: 'float16',
  batch_size: 16
})

/**
 * 启动/恢复转录
 */
async function startTranscription() {
  const jobId = store.meta.jobId

  // 1. 参数校验（如果有断点）
  const canProceed = await validateResume(jobId, currentSettings.value)
  if (!canProceed) {
    return  // 校验失败，中止启动
  }

  // 2. 连接SSE流
  connect(jobId)

  // 3. 调用后端启动接口
  try {
    await fetch(`/api/start`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        job_id: jobId,
        settings: JSON.stringify(currentSettings.value)
      })
    })

    store.task.status = 'transcribing'
    ElMessage.success('转录已启动！')
  } catch (error) {
    ElMessage.error('启动失败：' + error.message)
    disconnect()
  }
}
</script>
```

-----

## 5\. 完整的组件文件目录结构 (优化版)

```
src/
├── api/
│   ├── backend-peaks.js    # 新增：获取波形峰值数据
│   └── ...
├── assets/
│   └── styles/
│       ├── _variables.scss # 定义颜色变量、暗黑模式变量
│       ├── _mixins.scss    # 响应式断点混合宏
│       └── main.scss       # 全局入口
├── components/
│   ├── editor/
│   │   ├── VideoStage/
│   │   │   ├── index.vue   # 视口容器
│   │   │   └── OverlayLayer.vue # 字幕覆盖层 (抽离出来以复用)
│   │   ├── Timeline/
│   │   │   ├── index.vue   # 波形容器
│   │   │   └── WaveformMap.vue # (可选) 迷你导航图
│   │   ├── ScriptEditor/
│   │   │   ├── ListMode.vue
│   │   │   └── DocumentMode.vue
│   │   └── Controls/
│   │       ├── PlaybackBar.vue # 含自定义倍速
│   │       └── TimeDisplay.vue
│   ├── common/
│   │   ├── ThemeSwitch.vue # 主题切换器
│   │   └── ErrorBoundary.vue # 错误边界组件 (Vue 3)
├── composables/
│   ├── useTheme.js         # 管理深色/浅色模式
│   ├── useWaveform.js      # 封装 wavesurfer 复杂逻辑
│   ├── useAutoSave.js      # 自动保存逻辑
│   ├── useTranscribeStream.js  # 新增：SSE流式连接封装
│   └── useResumeValidation.js  # 新增：断点恢复校验逻辑
├── stores/
│   ├── projectStore.js     # 新增：项目核心数据管理
│   └── modelStore.js       # 已存在：模型管理
├── workers/
│   └── subtitle-merge.worker.js  # 新增：字幕合并Web Worker
└── views/
    └── ...
```

-----

## 6\. 开发路线图 (Revised)

### 第一阶段：基础设施与原型 (MVP)

1.  搭建 Vite + Vue 3 + Pinia + SCSS 脚手架。
2.  配置 `useDark` 实现基本的深色模式切换。
3.  实现后端 `/peaks` 接口（Python 端），确保大文件波形秒开。

### 第二阶段：核心编辑器 (Core)

1.  实现 `VideoStage` 与 `SubtitleList` 的基础联动（点击跳转）。
2.  集成 `wavesurfer.js`，实现**后端 Peaks 数据加载**。
3.  实现**双向绑定**：Region 拖拽 \<-\> Store 数据更新。

### 第三阶段：体验增强 (Polish)

1.  **响应式适配**：调试 Tablet 尺寸下的布局折叠逻辑。
2.  **自定义倍速**：实现 `input[type=number]` 控制 `playbackRate`。
3.  **鲁棒性建设**：添加自动保存和错误拦截。

这份终极版文档不仅涵盖了界面设计，更深入到了**大文件性能优化**、**样式工程化**和**异常兜底**等生产环境必须考虑的细节，是一个成熟软件产品的开发蓝图。

-----

## 7\. 核心算法实现：Web Worker 多线程合并

### 7.1 新增文件：`src/workers/subtitle-merge.worker.js`

为了避免在合并几千条字幕时阻塞主线程（导致界面卡顿），我们将"智能合并"逻辑放入后台线程。

```javascript
// src/workers/subtitle-merge.worker.js

/**
 * 字幕合并 Web Worker
 * 处理大量字幕数据的对齐合并操作，避免阻塞主线程
 */
self.onmessage = (e) => {
  const { currentSubtitles, alignedSegments } = e.data;

  try {
    // 创建一个 Map 以便快速查找 (O(1) 复杂度)
    const alignedMap = new Map(alignedSegments.map(s => [s.id, s]));

    const merged = currentSubtitles.map(item => {
      // 尝试在对齐结果中找到对应 ID 的片段
      const aligned = alignedMap.get(item.id);

      if (!aligned) return item;

      // 核心逻辑：用户改过的(Dirty)，保留文字只改时间；没改过的，全覆盖
      if (item.isDirty) {
        return {
          ...item,
          start: aligned.start,
          end: aligned.end,
          status: 'confirmed'
          // text: item.text (保留用户修改)
        };
      } else {
        return {
          ...item,
          start: aligned.start,
          end: aligned.end,
          text: aligned.text,
          status: 'confirmed'
        };
      }
    });

    // 返回合并结果
    self.postMessage(merged);
  } catch (error) {
    // 发送错误信息
    self.postMessage({ error: error.message });
  }
};
```

### 7.2 更新 Store 逻辑：集成 Web Worker

在 `stores/projectStore.js` 中修改 `mergeAlignedSubtitles` action，集成 Web Worker 处理逻辑：

```javascript
// stores/projectStore.js

import { defineStore } from 'pinia';

export const useProjectStore = defineStore('project', {
  state: () => ({
    // ...(保持前面文档中定义的完整state结构)
    meta: { /* ... */ },
    task: { /* ... */ },
    subtitles: [],
    player: { /* ... */ },
    view: { /* ... */ }
  }),

  actions: {
    // ...(保持其他actions)

    /**
     * SSE流式数据接收 Action（更新版）
     * 处理从后端推送过来的 'segment' (未对齐) 和 'aligned' (已对齐) 事件
     */
    handleStreamEvent(event) {
      if (event.type === 'segment') {
        // 流式追加：直接在主线程操作（量小，不卡）
        const newSegments = event.data.segments.map(seg => ({
          ...seg,
          status: 'draft',
          isDirty: false
        }));
        this.subtitles.push(...newSegments);

        // 更新进度条
        this.task.progress = event.data.progress.percentage;
        this.task.currentSegment = event.data.progress.processed;
        this.task.totalSegments = event.data.progress.total;
      }
      else if (event.type === 'aligned') {
        // 对齐完成：数据量大，交给 Worker 处理
        this.mergeAlignedSubtitles(event.data.segments);
      }
      else if (event.type === 'connected') {
        this.task.streaming = true;
      }
    },

    /**
     * 使用 Web Worker 进行智能合并
     * "一步到位"的生产级写法
     */
    async mergeAlignedSubtitles(alignedSegments) {
      this.task.status = 'aligning';

      try {
        // 动态创建 Worker 实例（使用 Blob 方式，避免路径问题）
        const workerCode = `
          self.onmessage = (e) => {
            const { currentSubtitles, alignedSegments } = e.data;
            try {
              const alignedMap = new Map(alignedSegments.map(s => [s.id, s]));
              const merged = currentSubtitles.map(item => {
                const aligned = alignedMap.get(item.id);
                if (!aligned) return item;
                if (item.isDirty) {
                  return {
                    ...item,
                    start: aligned.start,
                    end: aligned.end,
                    status: 'confirmed'
                  };
                } else {
                  return {
                    ...item,
                    start: aligned.start,
                    end: aligned.end,
                    text: aligned.text,
                    status: 'confirmed'
                  };
                }
              });
              self.postMessage(merged);
            } catch (error) {
              self.postMessage({ error: error.message });
            }
          };
        `;

        const workerBlob = new Blob([workerCode], { type: 'application/javascript' });
        const worker = new Worker(URL.createObjectURL(workerBlob));

        // 发送数据给 Worker (后台线程)
        const mergedResult = await new Promise((resolve, reject) => {
          worker.postMessage({
            // 必须传递纯数据对象 (序列化 Pinia 的响应式对象)
            currentSubtitles: JSON.parse(JSON.stringify(this.subtitles)),
            alignedSegments: alignedSegments
          });

          worker.onmessage = (e) => {
            if (e.data.error) {
              reject(new Error(e.data.error));
            } else {
              resolve(e.data);
            }
          };

          worker.onerror = (err) => reject(err);
        });

        // 接收结果，更新状态
        this.subtitles = mergedResult;
        this.task.phase = 'align';
        this.task.status = 'finished';

        // 任务完成，销毁线程
        worker.terminate();
        URL.revokeObjectURL(workerBlob);

      } catch (error) {
        console.error("Worker合并失败，回退到主线程合并:", error);
        // 兜底：主线程同步合并
        this.subtitles = this.subtitles.map(item => {
          const aligned = alignedSegments.find(a => a.id === item.id);
          if (!aligned) return item;

          if (item.isDirty) {
            return {
              ...item,
              start: aligned.start,
              end: aligned.end,
              status: 'confirmed'
            };
          } else {
            return {
              ...item,
              start: aligned.start,
              end: aligned.end,
              text: aligned.text,
              status: 'confirmed'
            };
          }
        });
        this.task.phase = 'align';
        this.task.status = 'finished';
      }
    }

    // ...(保持 loadCheckpoint、validateSettingsChange 等其他方法)
  }
});
```

### 7.3 新增 SSE 连接 Composable

创建 `composables/useTranscribeStream.js` 用于封装 SSE 连接逻辑：

```javascript
// composables/useTranscribeStream.js

import { ref, onUnmounted } from 'vue';
import { useProjectStore } from '@/stores/projectStore';
import { ElMessage } from 'element-plus';

/**
 * SSE流式转录监听 Composable
 * 自动处理重连、错误恢复等
 */
export function useTranscribeStream() {
  const store = useProjectStore();
  const eventSource = ref(null);
  const isConnected = ref(false);

  /**
   * 连接SSE流
   * @param {string} jobId - 任务ID
   */
  function connect(jobId) {
    disconnect();  // 先断开旧连接

    const url = `/api/stream/${jobId}`;
    eventSource.value = new EventSource(url);

    eventSource.value.onopen = () => {
      console.log(`[SSE] 连接已建立: ${url}`);
      isConnected.value = true;
    };

    eventSource.value.onmessage = (event) => {
      // 心跳检测
      if (event.data === 'heartbeat') return;

      try {
        const payload = JSON.parse(event.data);
        // 分发给 Store 处理
        handleMessage(payload);
      } catch (err) {
        console.error('[SSE] 消息解析失败:', err);
      }
    };

    eventSource.value.onerror = (error) => {
      console.error('[SSE] 连接错误:', error);
      isConnected.value = false;

      // 简单的重连逻辑（可选）
      // EventSource 会自动重连，这里可以添加额外的错误处理
    };
  }

  /**
   * 处理SSE消息
   */
  function handleMessage(data) {
    switch (data.type) {
      case 'connected':
        isConnected.value = true;
        store.task.streaming = true;
        console.log('[SSE] 连接确认');
        break;

      case 'segment':
        // 新片段转录完成 - 流式追加
        store.handleStreamEvent(data);
        ElMessage({
          message: `已转录 ${data.data.progress.processed}/${data.data.progress.total} 段`,
          type: 'success',
          duration: 1000,
          showClose: false
        });
        break;

      case 'aligned':
        // 对齐完成 - 批量更新
        store.handleStreamEvent(data);
        ElMessage({
          message: '✅ 对齐完成！字幕已更新',
          type: 'success',
          duration: 3000
        });
        break;

      case 'error':
        console.error('[SSE] 错误事件:', data.message);
        ElMessage.error(`转录错误: ${data.message}`);
        break;

      default:
        console.warn('[SSE] 未知事件类型:', data.type);
    }
  }

  /**
   * 断开连接
   */
  function disconnect() {
    if (eventSource.value) {
      eventSource.value.close();
      eventSource.value = null;
      isConnected.value = false;
      console.log('[SSE] 已断开');
    }
  }

  // 组件卸载时自动断开
  onUnmounted(() => {
    disconnect();
  });

  return {
    connect,
    disconnect,
    isConnected
  };
}
```

### 7.4 新增断点恢复校验 Composable

创建 `composables/useResumeValidation.js` 处理恢复任务时的参数冲突：

```javascript
// composables/useResumeValidation.js

import { ElMessageBox, ElMessage } from 'element-plus';
import { useProjectStore } from '@/stores/projectStore';

/**
 * 断点续传参数校验 Composable
 * 负责处理恢复任务时的参数冲突（包含 UI 交互）
 */
export function useResumeValidation() {
  const store = useProjectStore();

  /**
   * 校验并恢复参数
   * @param {string} jobId - 任务ID
   * @param {Object} currentSettings - 当前UI面板的设置
   * @returns {Promise<boolean>} - 是否允许继续（false表示需要用户重新确认）
   */
  async function validateAndRestore(jobId, currentSettings) {
    // 1. 获取后端 Checkpoint 数据
    const checkpoint = await store.loadCheckpoint(jobId);
    if (!checkpoint || !checkpoint.has_checkpoint) {
      return true; // 无断点，直接继续
    }

    // 2. 校验逻辑
    const result = store.validateSettingsChange(
      currentSettings,
      checkpoint.original_settings
    );

    // 3. 处理红线参数 (Immutable)
    if (result.warningLevel === 'error') {
      await ElMessageBox.alert(
        '检测到关键参数（设备/时间戳格式）变更，将强制恢复为原设置以确保数据一致性。\n\n' +
        result.immutableChanges.map(c =>
          `• ${c.key}: ${c.new} → ${c.old}`
        ).join('\n'),
        '参数冲突',
        {
          type: 'error',
          confirmButtonText: '确定'
        }
      );

      // 强制覆盖为原值
      result.immutableChanges.forEach(change => {
        currentSettings[change.key] = change.old;
      });
      return false; // 指示调用方刷新 UI
    }

    // 4. 处理黄线参数 (Warning)
    if (result.warningLevel === 'warning') {
      try {
        await ElMessageBox.confirm(
          '⚠️ 检测到模型参数已修改：\n\n' +
          result.warnChanges.map(c =>
            `• ${c.key}: ${c.old} → ${c.new}`
          ).join('\n') +
          '\n\n修改模型会导致前后字幕质量不一致！\n' +
          '适用场景：发现用错了模型需要中途更正。\n\n' +
          '是否继续使用新模型？',
          '严重警告',
          {
            confirmButtonText: '继续（使用新模型）',
            cancelButtonText: '恢复原模型',
            type: 'warning',
            dangerouslyUseHTMLString: false
          }
        );
        return true; // 用户确认，允许继续
      } catch {
        // 用户取消，恢复原设置
        result.warnChanges.forEach(change => {
          currentSettings[change.key] = change.old;
        });
        ElMessage.info('已恢复为原始模型配置');
        return false;
      }
    }

    // 5. 处理绿线参数 (Info)
    if (result.warningLevel === 'info') {
      ElMessage.info(
        `检测到参数修改：${result.freeChanges.map(c => c.key).join(', ')}`
      );
      return true;
    }

    return true; // 无变更，直接继续
  }

  return {
    validateAndRestore
  };
}
```

### 7.5 在 EditorView 中集成使用示例

```vue
<script setup>
import { ref, onMounted } from 'vue';
import { useProjectStore } from '@/stores/projectStore';
import { useResumeValidation } from '@/composables/useResumeValidation';
import { useTranscribeStream } from '@/composables/useTranscribeStream';
import { ElMessage } from 'element-plus';

const store = useProjectStore();
const { validateAndRestore } = useResumeValidation();
const { connect, disconnect } = useTranscribeStream();

const currentSettings = ref({
  model: 'medium',
  device: 'cuda',
  word_timestamps: false,
  compute_type: 'float16',
  batch_size: 16,
  cpu_affinity_enabled: false,
  cpu_affinity_strategy: 'auto'
});

/**
 * 启动/恢复转录
 */
async function startTranscription() {
  const jobId = store.meta.jobId;

  // 1. 参数校验（如果有断点）
  const canProceed = await validateAndRestore(jobId, currentSettings.value);
  if (!canProceed) {
    return; // 校验失败，中止启动
  }

  // 2. 连接SSE流
  connect(jobId);

  // 3. 调用后端启动接口
  try {
    const response = await fetch('/api/start', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        job_id: jobId,
        settings: currentSettings.value
      })
    });

    if (!response.ok) {
      throw new Error(`HTTP ${response.status}`);
    }

    store.task.status = 'transcribing';
    ElMessage.success('转录已启动！');
  } catch (error) {
    ElMessage.error('启动失败：' + error.message);
    disconnect();
  }
}

/**
 * 暂停转录
 */
async function pauseTranscription() {
  try {
    await fetch(`/api/pause/${store.meta.jobId}`, { method: 'POST' });
    store.task.status = 'paused';
    disconnect();
    ElMessage.info('任务已暂停');
  } catch (error) {
    ElMessage.error('暂停失败：' + error.message);
  }
}

onMounted(() => {
  // 如果有断点，可以在此处提示用户
  if (store.task.hasCheckpoint) {
    ElMessage({
      message: `检测到未完成的任务 (${store.task.progress.toFixed(1)}%)，点击"继续"可恢复`,
      type: 'info',
      duration: 5000
    });
  }
});
</script>

<template>
  <div class="editor-view">
    <!-- 工具栏 -->
    <div class="toolbar">
      <el-button
        v-if="store.task.status === 'idle' || store.task.status === 'paused'"
        type="primary"
        @click="startTranscription"
      >
        {{ store.task.hasCheckpoint ? '继续转录' : '开始转录' }}
      </el-button>
      <el-button
        v-if="store.task.status === 'transcribing'"
        @click="pauseTranscription"
      >
        暂停
      </el-button>
    </div>

    <!-- 其他组件... -->
  </div>
</template>
```

-----

## 8\. 总结：生产级特性清单

完成以上修改后，你的前端将具备以下能力：

1. **职责分离**：后端文档管 API，前端文档管实现，边界清晰。

2. **性能无忧**：
   - 通过 **Web Worker** 处理大量字幕合并，即使对齐 2 小时电影字幕，界面也不会卡顿。
   - 使用 Map 数据结构优化查找性能（O(1) 复杂度）。

3. **逻辑闭环**：
   - SSE 监听 → 状态更新 → Worker 处理 → 界面渲染，完整的数据流。
   - 参数校验 → 断点恢复 → 流式追加 → 智能合并，完整的业务流程。

4. **错误兜底**：
   - Worker 失败自动回退到主线程处理。
   - SSE 连接错误自动重连（EventSource 内置机制）。

5. **用户体验**：
   - 参数冲突时的三级警告（红/黄/绿线）。
   - 实时进度提示和完成通知。
   - 断点续传无缝衔接。

6. **代码质量**：
   - 所有路径使用相对路径（`@/stores/...`），符合项目要求。
   - 使用 Blob URL 创建 Worker，避免外部文件路径问题。
   - 完整的错误处理和日志记录。

**下一步建议**：
- 根据本文档创建 `composables/useTranscribeStream.js` 和 `useResumeValidation.js` 文件。
- 如需独立的 Worker 文件，创建 `workers/subtitle-merge.worker.js`（可选，当前实现已使用 Blob 方式）。
- 在实际组件中集成并测试 SSE 流式更新和断点恢复功能。