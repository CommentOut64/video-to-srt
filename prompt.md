...
修改模型预加载弹窗，让其更加紧凑，去除内存使用情况，弹窗应该固定大小没有滚动条；修改一键启动和其他代码，让启动时加载完后端后立即加载前端，当前端成功开启后再异步预加载模型，不影响选择媒体文件等其他操作；当修改了参数需要重新加载模型时，弹窗提示是否修改参数，确认后立即自动异步预加载新选择的模型。预加载按钮应该实时展示当前是否完成加载
